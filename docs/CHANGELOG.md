...
- [修复] 硬风险清仓改为结构化确认并过滤否定、假设和数据缺失文本，保留机器周期阶段并记录决策覆盖来源
- [改进] 技术性清仓收紧为极高位、完整已收盘日线和三类独立衰竭证据，高位破位及不完整数据默认最高减仓
- [改进] 飞书大盘文字推送改用结构化精简摘要，完整 Markdown 继续用于历史、文件和其他报告渠道
...
- [修复] 加速下跌优先于低效率横盘判定，避免冲高回落后的急跌被误标为横盘观察。
- [改进] 飞书策略报告压缩模板控制语句产生的连续空行，保留分隔线并减少消息纵向占用。
...
- [修复] 多维择时结果缺少策略字段时按评分与真实多维证据补齐六类信号，稳定性护栏降级时同步更新公开动作字段，避免报告上下结论冲突。
- [改进] 飞书聚合文字通知改用策略优先精简报告，保留完整 Markdown 报告与文件发送模式，减少多股日报的冗长分片。
- [改进] 每日 GitHub Actions 显式映射 Agent 与报告运行配置，并默认启用模板渲染，Agent 模式仍保持默认关闭。
