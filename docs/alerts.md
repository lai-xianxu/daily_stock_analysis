...
仓库自带 `.github/workflows/00-daily-analysis.yml` 是一次性分析 workflow：完整模式调用 `python main.py` 并同时生成个股分析和大盘复盘，仅大盘模式调用 `python main.py --market-review`。它不运行 `--schedule` 后台 alert worker，也没有映射 `AGENT_EVENT_*` 变量。仅在 repository Secrets / Variables 中新增 `AGENT_EVENT_MONITOR_ENABLED` 或 `AGENT_EVENT_ALERT_RULES_JSON` 不会让默认 Actions 开始持续轮询告警。
