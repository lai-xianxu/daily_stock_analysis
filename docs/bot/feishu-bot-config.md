...
聚合日报默认发送反向周期择时精简版，按清仓、减仓、抢筹、低吸、观察、持有排序，展示周期阶段、价格分位、量能、动能确认、多维约束和动作对应点位，不展示兼容评分。完整 Markdown 仍保存在本地报告或 GitHub Actions artifact；需要在飞书里直接接收完整内容时，使用下面的文件发送模式。
