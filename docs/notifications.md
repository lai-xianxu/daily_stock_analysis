...
聚合任务仍先生成并保存完整 Markdown 报告，邮件、Telegram 等渠道继续使用各自既有发送路径。飞书文字模式使用周期择时精简报告，按清仓、减仓、抢筹、低吸、观察、持有排序，隐藏 `sentiment_score`，并为不同动作展示介入/失效、压力/退出、箱体或防守点位。完整规则见 [反向周期择时策略](contrarian-cycle-timing.md)。`FEISHU_SEND_AS_FILE=true` 时仍发送完整 `.md` 文件。

大盘复盘同时保存完整 Markdown 和结构化 `market_review_payload.notification_digest`。飞书文字使用约 600-900 字摘要，固定展示市场状态与策略、涨跌家数/涨跌停/成交额、指数结构、主线与弱项、维持/降档/防守条件、最大风险和数据限制，不再发送指数 OHLC 大表、Top 5 双表或三套重复仓位建议。独立大盘推送、缓存复用和个股/大盘合并推送读取同一摘要；其他渠道继续收到完整报告。飞书文件模式忽略文字覆盖，仍上传完整 Markdown。

仓库自带每日 GitHub Actions workflow 默认注入 `REPORT_RENDERER_ENABLED=true`，本地与 Docker 的全局默认值仍为 `false`；两者均可显式覆盖。模板渲染失败时继续走现有兼容 fallback，不影响分析任务和其他通知渠道。

兼容性排除说明：
- 未改动 `src/notification_sender/wechat_sender.py`、`src/notification_sender/slack_sender.py`、`src/notification_sender/telegram_sender.py` 的发送实现；飞书 sender 的 Webhook/App Bot 传输逻辑不变，仅由 pipeline 为文字报告选择精简内容。
...
- `REPORT_SHOW_LLM_MODEL` 与 `report_renderer_enabled` 均为展示/降级策略开关：关闭仅影响报告可见结构，不会触发配置迁移或运行时参数回退。
