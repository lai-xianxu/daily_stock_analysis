...
聚合任务仍先生成并保存完整 Markdown 报告，邮件、Telegram 等渠道继续使用各自既有发送路径。飞书文字模式改用与企业微信同源的精简策略报告，优先展示六类策略信号、3 条多维理由、关键量价、主要风险及升级/失效条件，避免完整仪表盘被拆成大量长消息。`FEISHU_SEND_AS_FILE=true` 时仍发送完整 `.md` 文件。
